...
export class ExamModeUI extends UIScreen {
  ...
  private _examStartButtonEnableInterval: ReturnType<
    typeof setInterval
  > | null = null;
  ...
  public constructor(auth: Authenticator, loginUI: LoginScreenUI) {
    ...
    this._form = {
      ...
      examProjectsText: document.getElementById(
        "exam-mode-projects",
      ) as HTMLSpanElement,
      examStartText: document.getElementById(
        "exam-mode-start",
      ) as HTMLSpanElement,
      ...
      examStartTimer: document.getElementById(
        "exam-mode-start-timer",
  ...
  private _populateData(examsToPopulate: ExamForHost[]): void {
    ...
    if (examsToPopulate.length === 0) {
      ...
      const exams = window.data.dataJson?.exams.filter((exam) =>
        examsToPopulate.some((examToPopulate) => exam.id === examToPopulate.id),
      );
      ...
      const projectsText = exams
        .flatMap((exam) => exam.projects.map((project) => project.name))
        .join(", ");

      // Display the projects and the time range in which the exams can be started
      form.examProjectsText.innerText = projectsText;
      form.examStartText.innerText = earliestExam.toLocaleTimeString("en-NL", {
        hour: "2-digit",
        minute: "2-digit",
      });
      form.examEndText.innerText = latestExam.toLocaleTimeString("en-NL", {
        hour: "2-digit",
        minute: "2-digit",
      });
      ...
      if (this._examStartTime.getTime() > Date.now()) {
        this._examStartButtonEnableInterval = setInterval(() => {
          const timeLeft = Math.floor(
            (this._examStartTime.getTime() - Date.now()) / 1000,
          );
